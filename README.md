...
The Harbor Scanner Adapter for Aqua CSP is a service that translates the Harbor scanning API into `scannercli` commands
and allows Harbor to use Aqua CSP ad hoc scanning for providing vulnerability reports on images stored in Harbor
...
the Aqua server to fulfil the contract required by Harbor scanning API. This contract is based on OS and application
...
To see the whole report generated by the Aqua scanner, including detected *malware* and *sensitive data*, you have
to navigate from Harbor Portal to Aqua Management Console. In other words, displaying malware or sensitive data in
Harbor Portal requires changes to the Pluggable Scanners API, which is owned by the Harbor community and contributed to
by other security vendors.

Similarly, proprietary *image assurance policies* that can be configured in the Aqua Management Console are not
integrated with the Harbor deployment security checker through Pluggable Scanners API. Therefore, Aqua assurance
policies are not merged, displayed nor enforced by Harbor in any way.
